// From `rust-ibverbs` crate by `jonhoo`.
// Original file at https://github.com/jonhoo/rust-ibverbs/blob/3c1c57d1c3e1fa2dd00390174ac4bf70605e123f/ibverbs-sys/build.rs
...
fn main() {
    ...
    let output_path = manifest_dir.join(BINDINGS);

    // Ensure the parent directory exists
    if let Some(parent) = output_path.parent() {
        fs::create_dir_all(parent).expect("Failed to create ibverbs bindings output directory");
    }

    // Write the bindings to the file
    bindings
        .write_to_file(&output_path)
        .unwrap_or_else(|_| panic!("Couldn't write ibverbs bindings to {BINDINGS}!"));

    println!("cargo:warning=Generated ibverbs bindings at {BINDINGS}");
}
